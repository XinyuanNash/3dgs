	...
	bool antialiasing)
{
	...
	if(antialiasing)
	{
		det_cov = c_xx * c_yy - c_xy * c_xy;
		add_blur(h_var, c_xx, c_yy, c_xy, h_convolution_scaling, antialiasing);
		...
		const float ratio = det_cov / det_cov_plus_h_cov;
		// Skip the d_inside_root when the ratio is degenerate (matches LFS:
		// sqrt(max(0.f, ratio)) gives 0 when ratio <= 0, so its derivative
		// through the chain rule is also 0 there).
		d_inside_root = (ratio <= 0.f) ? 0.f : d_h_convolution_scaling / (2 * h_convolution_scaling);
	}
	else
	{
		c_xx += h_var;
